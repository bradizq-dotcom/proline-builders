...
export default function GetAQuote() {
  ...
  const buildMailto = () => {
    const lines = [
      ...
      `Phone: ${form.phone}`,
      `Email: ${form.email}`,
      `Decision Maker: ${form.decisionMaker === 'yes' ? 'Yes' : 'No'}`,
      `Plans Available: ${form.plansAvailable === 'yes' ? 'Yes' : 'Not yet'}`,
      `Property Type: ${form.propertyType}`,
      `Location: ${form.location}`,
      `Budget: ${form.budget}`,
      `Scope: ${form.scope.join(', ')}`,
      `Timeline: ${form.timeline}`,
      form.files.length > 0 ? `Files to attach manually: ${form.files.map(f => f.name).join(', ')}` : 'Files: none',
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6 pt-20">
        <div className="max-w-lg text-center">
          ...
          <p className="text-slate-500 mb-2">Thanks, {form.name} — your email app should have opened with your request pre-filled. Just hit send and we'll respond within one business day.</p>
          <p className="text-slate-400 text-sm mb-8">Email app didn't open? <a href={buildMailto()} className="text-blue-700 font-semibold hover:underline">Click here to open it manually</a>.</p>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-sm border border-gray-100 p-8 shadow-sm">
          {step === 0 && (<div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Email *</label><input required type="email" value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} placeholder="you@example.com" className="w-full border border-gray-200 rounded-sm px-4 py-3 text-slate-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-colors text-sm" /></div>
            </div>
          </div>)}
          ...
          {step === 4 && (<div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-sm p-4 text-sm text-blue-800"><p>Clicking <strong>Submit Request</strong> opens your email app with your details pre-filled — just hit send and we'll respond within one business day.</p></div>
            <button type="submit" className="w-full bg-blue-700 hover:bg-blue-800 text-white font-bold py-4 rounded-sm uppercase text-sm tracking-wider transition-all flex items-center justify-center gap-2">Submit Request <ArrowRight className="w-4 h-4" /></button>
          </div>)}
